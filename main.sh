...
if [ $archserver == "1" ]; then
    echo "Arch Linux Server installed, skipping graphical environments and standard app suite!"
    exit 0
fi
...
if [ $de_wm == "D" ]; then  # DE: KDE
    ...
    services="bluetooth.service firewalld.service cups.service"
    init_enable
    ...
    services="bluetooth.service ufw.service cups.service"
    init_enable
